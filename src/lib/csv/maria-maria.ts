...
import type { ExternalWeeklyRow } from "../server/external";

export function reconstructMariaMaria(
  metaDataset: ParsedDataset,
  externalWeekly: ExternalWeeklyRow[],
): MariaMariaDataset {
  const weeks: MariaMariaRow[] = [];

  // O dataset do Meta pode ser consolidado ou diário
  const isConsolidated =
    metaDataset.rows.length < 50 && metaDataset.rows.some((r) => r.date !== r.endDate);
  ...
  for (const r of metaDataset.rows) {
    const s = parseDate(r.date);
    const e = parseDate(r.endDate || r.date);
    if (s > 0 && s < reportRange.start) reportRange.start = s;
    if (e > 0 && e > reportRange.end) reportRange.end = e;
  ...
  for (const row of externalWeekly) {
    // start_date e end_date vêm no formato YYYY-MM-DD
    ...
    if (isConsolidated) {
      ...
      for (const mr of metaRows) {
        ...
        metaAgg.clicks += mr.clicks;
        metaAgg.conversations += mr.conversations;
        metaAgg.ctrSum += mr.ctr * mr.impressions;
        metaAgg.ctrTodosSum += (mr.ctrTodos || 0) * mr.impressions;
        metaAgg.cpmSum += mr.cpm * mr.impressions;
        metaAgg.frequencySum += mr.frequency * mr.impressions;
        metaAgg.count += mr.impressions;
    ...
    // Formata o texto da semana (ex: DD/MM a DD/MM)
    const startDateObj = new Date(row.startDate + "T00:00:00");
    const endDateObj = new Date(row.endDate + "T23:59:59");
    const formatDayMonth = (d: Date) => {
      const day = String(d.getDate()).padStart(2, "0");
      const month = String(d.getMonth() + 1).padStart(2, "0");
      return `${day}/${month}`;
    };
    const semanaText = `${formatDayMonth(startDateObj)} a ${formatDayMonth(endDateObj)}`;
    ...
    const mmRow: MariaMariaRow = {
      ...
      endDate: end,
      salonData: {
        contatosWhatsapp: row.contatosWhatsapp,
        agendamentos: row.agendamentos,
        agendamentosComServico: row.agendamentosComServico,
        totalFaturamento: row.faturamento,
        ticketMedio: row.ticketMedio,
      },
      metaData: finalMetaData,
      roasReal: finalMetaData.spend > 0 ? row.faturamento / finalMetaData.spend : 0,
      cacReal:
        row.agendamentosComServico > 0 ? finalMetaData.spend / row.agendamentosComServico : 0,
      taxaConversaoReal:
        row.contatosWhatsapp > 0 ? (row.agendamentosComServico / row.contatosWhatsapp) * 100 : 0,
    };
